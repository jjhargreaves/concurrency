...
void particle(chanend left, chanend right, chanend toVisualiser, int startPosition, int startDirection, int id) {
	...
	while(gameRunning)
	{
			...
			if(id == 0)
			{
				...
				if((rightAttempt != attemptedPosition) && (leftAttempt != attemptedPosition) && ( (((moveCounter%(id+1)) == 0) || (velocityFlag))) &&
						(directionChanged == 0) && (attemptedPosition != rightIntent.position) && (attemptedPosition != leftIntent.position))
				{
					currentPosition = (currentPosition + currentDirection +12)%12;
				}
